...
\newcommand{\rquestionII}{Which minimal set of sensorimotor modalities is necessary and sufficient to enable the autonomous learning of fundamental properties of the robot morphology?}
\newcommand{\rquestionIII}{How can the sensorimotor relationships emerging from robot embodiment be exploited to support the gradual understanding and monitoring of the body morphology?}
...
%However, existing learning frameworks in robotics neither include nor provide information about the robot's body structure and instead focus on local or global approaches to capture input-output relationships \cite{NguyenTuong2011Modellearningrobot}. Local methods suffer from limited generalization and hyperparameter sensitivity \cite{Thrun2002Probabilisticrobotics,Goodfellow2016DeepLearning}, while global methods risk overfitting and computational overload. \redtext{Additionally, deep learning, the standard end-to-end learning framework, despite the advancements in computational power and data availability, still faces challenges, such as the difficulty in determining dedicated network architectures \cite{Baker2017Designingneuralnetwork,Elsken2019Neuralarchitecturesearch}, as well as issues with low sample efficiency, extended training times, and limited generalization \cite{Pierson2017Deeplearningrobotics,Suenderhauf2018limitspotentialsdeep}. In general, the challenges reported in the learning literature can often be associated with oversight or the deliberate neglect of prior principled knowledge.}
...
%In presenting the results, this study utilized alternative methods rooted in deep learning for comparison. The findings underscored that integrating principled knowledge effectively addresses challenges related to sample efficiency and generalization in end-to-end learning applications for robotic systems. It is crucial to emphasize that the intent here is not to diminish the remarkable achievements of deep learning frameworks. Rather, the critique concerning the often indiscriminate exclusion of principled and model-based knowledge serves as a call to recognize the limitations of pure black-box learning. The primary message underscores the significance of purposefully incorporating available knowledge, thereby reducing unnecessary complexities within intricate learning problems. \redtext{In fact, the recurring issues in learning approaches for embodied systems, such as dimensionality, limited generalization, sample efficiency, and computational demands, suggest an intractability gap inherent to these complex systems. This highlights the need to integrate principled knowledge to overcome the limitations of pure black-box learning.} The enhancement of the representational capabilities of state-of-the-art deep learning methods with structural knowledge paves the way for achieving genuine robot autonomy.
...
%This research introduced an approach to enhancing robot autonomy and self-awareness through the integration of proprioceptive signals. This comprehensive fusion of sensory inputs, encompassing both afferent and efferent signals, allowed robots to autonomously acquire, monitor, and adapt their body schema knowledge, eliminating the need for external sensors.
%
%The research restructured robot kinematic calibration and parametric system identification processes into a machine-learning-friendly framework. Consisting of a pipeline of modular incremental learning processes, the framework streamlined these processes and facilitated extraction of fundamental features of the robot body schema. By integrating proprioceptive signals with first-order principles inherent to the robot embodiment, this approach ensured the accuracy and robustness of the robot body schema representation.
%
%To characterize key morphological properties in tree-like floating base structures, the research utilized fundamental proprioceptive signals. The mechanical topology, initially inferred model-free through information-theoretic measures, was simultaneously validated and utilized to establish the robot's kinematic description without requiring external calibration devices. This approach ensured the accuracy of the body schema even in dynamic environments. Additionally, the research proposed online learning of inertial properties of links within the inferred morphology, considering their existence on the Riemannian manifold of symmetric positive definite matrices. This ensured continuous physical feasibility and paved the way for robust, adaptable, and intelligent robotic systems.
...
%The assumptions, methods, and claims presented in this dissertation are specifically applicable to the broad class of tree-like robotic structures with revolute joints and fixed or floating bases. It is important to note that parallel mechanisms and soft robots were beyond the scope of this study, and therefore, the applicability of the framework introduced here is explicitly left for future work.
%
%One aspect of the robot body schema learning pipeline, illustrated in Fig.~\redtext{XXX}, that remained unaddressed is the online generation of excitation motions in accordance with the learning context—a challenge known in the literature as the exploration problem. This is intricately linked to the motion policy the robot adheres to for achieving the estimation, as seen in reinforcement learning, for instance. Unlike the conventional design of excitation trajectories for the estimation of inertial parameters, challenges arise when learning the robot mechanical topology and kinematic description, as even a basic model of the robot structure and alternative methods need to be explored to not only define appropriate excitation motions but also to determine control strategies that allow the execution of such motions.
%
%Ensuring self-safety is a critical aspect during the learning of the physical self. While safety is commonly discussed within the context of human-robot interaction, for a robot in the process of discovering its morphology it is paramount to ensure physical integrity. It is worth mentioning that the methodology introduced in this dissertation does not address self-safety. Indeed, the risks of environmental and self contacts during virtual experiments were disregarded. Yet, the joint trajectories used in the real robot experiments were meticulously designed to avoid contacts. Certainly, especially for floating base robots, (environmental) contacts are not only unavoidable but also necessary. In general, without contingent strategies that ensure self-safety it is possible that exploratory motions during learning result in self-damage. This is a strong reason why the early stages of many learning frameworks are mostly executed on virtual environments. Nevertheless, developing contingent strategies to ensure safe bodily exploration remains an open question.
